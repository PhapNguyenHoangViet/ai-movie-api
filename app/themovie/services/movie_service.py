...
def get_max_values():
    try:
        ...
        logging.info(f"Max user ID: {MAX_USER}, Max movie ID: {MAX_ITEM}")
...
class MovieRecommender:
        ...
        try:
            ...
            for epoch in range(epochs):
                ...
                for data in train_loader:
                    ...
                    loss = torch.sqrt(
                        criterion(edge_scores, data.edge_attr.to(torch.float))
                    )
                    loss.backward()
                    optimizer.step()
    ...
    def update_with_new_ratings(
        self, new_ratings, feature_matrix, model_path, epochs=5
    ):
        if new_ratings.empty:
            logging.info("No new ratings to update.")
            return
        ...
        try:
            ...
            if "rating_id" in new_ratings.columns and not new_ratings.empty:
                ...
                logging.info("Updated ratings processed status.")
    ...
    def auto_update_model(self, model_path):
        try:
            ...
            if len(new_ratings) >= 5:
                logging.info(f"Found {len(new_ratings)} new ratings. Updating model...")
